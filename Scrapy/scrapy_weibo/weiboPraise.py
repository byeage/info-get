...
def weiboPraiseScrapy(id, page):
    ...
    try:
        ...
        print(data['list'])
        dbInstance.add_weiboPraise(data['list'])
        if hasNext:
            time.sleep(6)
            if len(errorPages) < 10:
                weiboPraiseScrapy(id, nextPage)
        else:
...
def weiboPraiseProcessErrorPages(id, page):
    """ 抓取微博评论
        :param id: 微博ID
        :param page: 评论翻页页数
        :return:
...
if __name__ == '__main__':
    id = 4230070799343854
    page = 1
    weiboPraiseScrapy(id, page)
    for p in errorPages:
        weiboPraiseProcessErrorPages(id, p)
